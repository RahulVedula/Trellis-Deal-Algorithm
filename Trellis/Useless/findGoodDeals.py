...
def main():
    ...
    for i in range(len(df)):
        ...
        # if changeInGrowthRate(profitBefore,profitAfter) > 0 and unitBeforeDeal.sum() > 5 and linAverage(profitBefore['profit']).intercept < linAverage(profitAfter['profit'].truncate(before = end_date, after = datetime.strptime(end_date, '%Y-%m-%d')+timedelta(days=14))).intercept:
        #     print(asinValue)
        #     if len(list_top_slopes) >= 10:   
        #        list_top_slopes.sort(key=lambda x: x[0]) 
        #        if (list_top_slopes[0][0] < changeInGrowthRate(profitBefore,profitAfter)):
        #             list_top_slopes[0] = [changeInGrowthRate(profitBefore,profitAfter),asinValue, start_date,end_date]
        #     else:
        #         list_top_slopes.append([changeInGrowthRate(profitBefore,profitAfter),asinValue,start_date,end_date])
